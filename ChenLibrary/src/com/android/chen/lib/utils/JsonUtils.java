...
/**
 * 这是一个简易的Json-JsonHashMap转换工具，可以将普通的json数据（字符串） 转换为一个JsonHashMap<Srting,
 * Object>表格，也可以反过来操作。此外还支 持将json数据格式化。
 */
public class JsonUtils {

	/** 将指定的json数据转成 {@link IMap}<String, Object>对象 */
	...
	/** 将指定的 {@link IMap}<String, Object>对象转成json数据 */
	public static String fromJsonHashMap(IMap map) {
		try {
	...
	public static JSONObject getJSONObject(IMap map) throws JSONException {
		JSONObject json = new JSONObject();
		for (Entry<String, Object> entry : map.entrySet()) {
			Object value = entry.getValue();
			if (value instanceof IMap) {
				value = getJSONObject((IMap) value);
	...
	@SuppressWarnings("unchecked")
	public static String format(String sepStr, IMap map) {
		...
		for (Entry<String, Object> entry : map.entrySet()) {
			...
			Object value = entry.getValue();
			if (value instanceof IMap) {
				sb.append(format(mySepStr, (IMap) value));
			} else if (value instanceof ArrayList<?>) {
